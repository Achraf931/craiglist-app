<template>
  <div class="city">
    <div class="containerBody">
      <h2 style="margin-bottom: 15px;">Offres</h2>
      <p v-if="offers.length <= 0">Aucune offre pour cette catégorie</p>
      <div v-else class="containerOffers">
        <transition name="fade" v-for="(offer, index) in offers" :key="index">
          <Offer class="offer" :offer="offer" :nameRoute="'offer'"/>
        </transition>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    beforeMount() {
      this.$store.dispatch('offers/getOffers', {page: 1, city: this.cityParam, category: this.categoryParam, subcategory: this.subCategoryParam})
    },
    methods: {
      getOffers(page) {
        this.$store.dispatch('offers/getOffers', {page: page, city: this.cityParam, category: this.categoryParam, subcategory: this.subCategoryParam})
      }
    }
  }
</script>
